...
//         width: document.getElementById('poster').clientWidth * scale,
//         height: document.getElementById('poster').clientHeight * scale,
//         style: {
...
function searchInfo(id){
    ...
    .then(function (response){
        ...
        document.getElementById('date').innerText = 'Release: ' + response.release_date.substring(0,4)
        document.getElementById('movie_name').innerText = response.title
    ...
    fetch('https://api.themoviedb.org/3/movie/' + id + '/credits?language=en-US', options)
        ...
        .then(function (response){
            ...
            for (let i = 0; i < response.crew.length; i++) {
                ...
                if(response.crew[i].job === 'Costume Design'){
                    list_costume.push(response.crew[i].name)
                }
            }
            document.getElementById('director0').innerText = list_director[0]
            ...
            document.getElementById('costume0').innerText = list_costume[0] ? list_costume[0] : ''
            document.getElementById('costume1').innerText = list_costume[1] ? list_costume[1] : ''
...
function searchTitle(el){
    fetch('https://api.themoviedb.org/3/search/movie?query=' + el.value + '&include_adult=false&language=fr-FR&page=1', options)
        .then(response => response.json())
        .then(function (response){
